...
export const inspirationApi = {
  getInspirationFeed: async (params: GetInspirationParams = {}): Promise<InspirationResponse> => {
    try {
      ...
      // Fallback if backend endpoint is not yet mounted: aggregate from community API
    }
 ...
 * and curated recommendations so that the inspiration feed is fully functional.
 */
async function aggregateInspirationFallback(params: GetInspirationParams): Promise<InspirationResponse> {
  const items: InspirationItem[] = [];

  try {
    const destinations = await communityApi.getDestinations(params.q);

    // Fetch public trips and recommendations across matching destinations
      ...
      destSlice.map(async (dest) => {
        const [trips, recs] = await Promise.allSettled([
          ...
          communityApi.getRecommendations(dest.id, {
            category: params.category && params.category !== 'all' && params.category !== 'itinerary'
              ? params.category
              : undefined,
          }),
        ]);
      ...
      if (!params.category || params.category === 'all' || params.category === 'itinerary') {
        for (const trip of trips) {
          ...
          const stages = (trip.stages || []).map((s) => ({
            ...
            country_name: s.country_name || dest.country_name,
            flag_emoji: s.flag_emoji || dest.flag_emoji,
            order_index: s.order_index,
            arrival_date: s.arrival_date,
            departure_date: s.departure_date,
          }));

          const coverPhoto = trip.photos?.find((p) => p.is_cover)?.photo_url || trip.photos?.[0]?.photo_url || dest.banner_url;
          ...
          items.push({
            ...
            author: {
              ...
              town_name: trip.author.town_name || 'Girona',
              region_name: trip.author.region_name,
              country_name: trip.author.country_name,
      ...
      for (const rec of recs) {
        ...
        items.push({
          ...
          author: {
            ...
            town_name: rec.author.town_name,
            region_name: rec.author.region_name,
            country_name: rec.author.country_name,
    ...
    console.error('Error fetching fallback inspiration items:', err);
  }

  // If no items were found (e.g. fresh DB), provide high-value curated inspiration seed items
  if (items.length === 0) {
  ...
  const seed: InspirationItem[] = [
    {
      ...
      description: 'Una immersió completa des del bullici tecnològic de Tòquio fins a la pau dels temples de Kyoto i la gastronomia urbana d’Osaka.',
      cover_image_url: 'https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?w=900&auto=format&fit=crop&q=80',
      flag_emoji: '🇯🇵',
      country_name: 'Japó',
      country_code: 'JP',
    ...
    },
    {
      ...
      description: 'El millor brou concentrat de peix i porc que tastareu mai. Arribeu 15 minuts abans d’obrir per evitar una hora de cua.',
    ...
    },
    {
      ...
      description: 'Cascades infinites, platges de sorra negra, glaceres mil·lenàries i banys termals naturals completament solitaris.',
      cover_image_url: 'https://images.unsplash.com/photo-1504893524553-b855bce32c67?w=900&auto=format&fit=crop&q=80',
      flag_emoji: '🇮🇸',
    ...
    },
    {
      ...
      description: 'Construïda el 1923 enmig d’una vall verda volcànica. Cal caminar 20 minuts pel riu; porteu roba d’abric per al canviador rústic.',
      image_url: 'https://images.unsplash.com/photo-1529963183134-61a90db47eaf?w=800&auto=format&fit=crop&q=80',
    ...
    },
    {
      ...
      description: 'No feu cues a l’aeroport per comprar la targeta física de transport; afegiu la Suica directament al mòbil i recarregueu amb targeta bancària.',
    ...
    },
    {
      ...
      description: 'Reserveu el vagó panoràmic al matí per gaudir de la llum sobre els glaciars i el viaducte de Landwasser sense reflexos.',
    ...
    {
      id: 'seed-rec-anecdote',
      type: 'recommendation',
      category: 'anecdote',
      title: 'Nit de te i música amaziga al desert de Merzouga',
      description: 'Ens vam perdre una mica buscant el campament, però una família local ens va convidar a prendre te de menta i ens van guiar tocant el darbouka.',
      image_url: 'https://images.unsplash.com/photo-1509316975850-ff9c5deb0cd9?w=800&auto=format&fit=crop&q=80',
      location_name: 'Dunes d’Erg Chebbi',
      destination_name: 'Marroc',
      flag_emoji: '🇲🇦',
      country_name: 'Marroc',
      country_code: 'MA',
      useful_votes_count: 27,
      user_has_voted: false,
      author: {
        id: 'author-reus-1',
        name: 'Sergi V.',
        town_name: 'Reus',
        region_name: 'Baix Camp',
      },
